...
const AllVendors = () => {
  ...
  const otherPhotographer = [
    {
      image: "https://i.pinimg.com/736x/48/dd/df/48dddf382438ce6d73df9b20ab1ccb14.jpg",
      name: "Vinay Photography",
      rating: "4.8",
      status: "Verified",
    },
    {
      image: "https://i.pinimg.com/736x/c2/91/7b/c2917b4fefb226b82e01d94e9b817aaf.jpg",
      name: "Sanjana Studio",
      rating: "4.9",
      status: "Non-Verified",
    },
    {
      image: "https://i.pinimg.com/736x/8c/81/bd/8c81bdef6dad4f2e9fccd05b6bcdb8fb.jpg",
      name: "Krishna Photo Lab",
      rating: "4.7",
      status: "Verified",
    },
    {
      image: "https://i.pinimg.com/736x/6c/0c/ad/6c0cadb659e56f800ec3926a8069c58d.jpg",
      name: "Anjali Photo Studio",
      rating: "4.6",
      status: "Non-Verified",
    },
    {
      image: "https://i.pinimg.com/736x/24/42/a0/2442a0ef6603cc778993373562bd3a4c.jpg",
      name: "Prince Photo Lab",
      rating: "5.0",
      status: "Verified",
    },
    {
      image: "https://i.pinimg.com/736x/12/9e/ee/129eeec2da4c4f9f1c814ed97b861374.jpg",
  ...
  return (
    <div className=" flex flex-col items-center  justify-start  max-w-[1280px] w-full min-h-[100vh] h-fit   px-2 gap-2 lg:gap-5 py-2 ">
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] w-full gap-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] w-full gap-10">
        <div className=" flex flex-col items-center justify-start gap-4 md:gap-2 w-full ">
          ...
          <div className="flex items-start justify-center w-full border h-48 md:h-60 lg:h-96 rounded-lg bg-black ">
            <div className=" flex flex-col items-center justify-center w-1/2 p-4 gap-2 md:gap-5 h-full text-white ">
              ...
              <h1 className=" text-lg  md:text-2xl lg:text-3xl xl:text-5xl ">
                {" "}
          ...
          <div className="flex flex-col items-start justify-between w-full h-fit gap-4 border border-gray-200 p-4 rounded">
            <h1 className="text-lg font-semibold">People also like</h1>

            <div className="flex items-start w-full gap-4 overflow-x-auto scrollbar-hide">
              {
                otherPhotographer.map((data, idx) => (
                  <div key={idx} className="flex flex-col items-center justify-start w-32 h-40 min-w-28  border border-gray-200 rounded shadow-sm bg-white relative">
                    <img
                      src={data.image}
                      alt={data.name}
                      className="w-full h-full  object-cover rounded"
                    />
                    <div className="absolute bottom-0 h-full w-full left-0 bg-gradient-to-t from-black/60 to-transparent z-50 flex items-end justify-end p-2 "><p className="text-sm text-white font-medium text-center">{data.name}</p></div>

                    {/* <p className="text-sm font-medium text-center">{data.name}</p>
                    <p className="text-xs text-gray-500">⭐ {data.rating}</p>
                    <p className={`text-xs mt-1 px-2 py-0.5 rounded-full ${data.status === "Verified" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                      {data.status}
                    </p> */}
                  </div>
                ))
          ...
          <div className=" flex flex-col items-start justify-start w-full gap-3 h-fit lg:shadow-md rounded-md  bg-[rgb(255,255,255)] lg:border border-gray-300 ">
        ...
        <div className="hidden lg:flex flex-col items-center justify-start w-full md:px-5 ">
